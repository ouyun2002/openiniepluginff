...
bool ScriptablePluginObject::Invoke(NPObject* obj, NPIdentifier methodName,
  ...
  if (!strcmp(name, kSayHello)) {
	...
	HINSTANCE hInstance = ShellExecuteA(NULL, "open", "C:\\Program Files\\Internet Explorer\\iexplore.exe", url, NULL, SW_SHOWNORMAL);
	int a = (int) hInstance;
	
	/*
	char buffer[255];
	sprintf(buffer, "result is %d", a);
	char* npOutString = (char *)npnfuncs->memalloc(strlen(buffer) + 1);
	if (!npOutString)
      return false;
	strcpy(npOutString, buffer);
